function out = FCV_EcoEMS_Plant_S(t,v,soc,a,p_fcs,ds,slope_s,m_H_is_0_0)
%% FCHEV plant in spatial domain.

% update states
v_new = sqrt(max(v.^2 + 2.*a.*ds,0.01));    % update v
v_mean= (v+v_new)./2;
% if a~=0
% v_mean = ((v.^2+2.*a.*ds).^(3/2)-v.^3) ./ (3.*a.*ds);
% end
v_mean = v_new;
dt = ds./v_mean;
...
F_roll = veh_mass .* veh_gravity .* (veh_rrc.*cos(slope_s)+sin(slope_s));
F_drag = alpha_a .* (v_mean.^2);
F_acc = veh_mass .* a;
P_dem_m = v_mean .* (F_acc + F_roll + F_drag);
% if a~=0
% P_dem_m = (F_acc+F_roll).*v_mean + alpha_a.*((v.^2+2.*a.*ds).^(5/2)-v.^5) ./ (5.*a.*ds);
% end
...
fc_fuel = interp1(fc_pwr_map, fc_fuel_map, p_fcs, 'linear', 'extrap');
end
...
soc_new = soc - ess_cur ./ ess_Q * dt;
soc_new = (conj(soc_new) + soc_new) ./ 2;

% infeasiable
I = uint32(inf_fc + inf_mc + inf_ess ~= 0);
...
E_dem_m = P_dem_m .* dt;
E_dem_e = mc_inpwr .* dt;
E_dem_m_pos = P_dem_m.*(P_dem_m>0) .* dt;
E_dem_e_pos = mc_inpwr.*(mc_inpwr>0) .* dt;
E_dem_m_pos_90neg = (P_dem_m.*(P_dem_m>0) + 0.9.*P_dem_m.*(P_dem_m<=0)) .* dt;
E_dem_e_pos_90neg = (mc_inpwr.*(mc_inpwr>0) + 0.9.*mc_inpwr.*(mc_inpwr<=0)) .* dt;
...
out.E_dem_m = E_dem_m; %
out.E_dem_e = E_dem_e; %
out.E_dem_m_pos = E_dem_m_pos; %
out.E_dem_e_pos = E_dem_e_pos; %
out.E_dem_m_pos_90neg = E_dem_m_pos_90neg; %
out.E_dem_e_pos_90neg = E_dem_e_pos_90neg; %
